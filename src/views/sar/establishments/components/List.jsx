...
function List() {
  ...
  const ActionCell = ({ rowData, dataKey, ...props }) => {
    function editEstablecimiento() {
      match(`/admin/sar/establecimiento/editar/${rowData[dataKey]}`);
    }
    function showEstablecimiento() {
      match(`/admin/sar/establecimiento/show/${rowData[dataKey]}`);
    }
    return (
      <Cell {...props} className="link-group">
        <IconButton appearance="subtle" onClick={editEstablecimiento} icon={<Edit2 />} />
        <Divider vertical />
        <IconButton appearance="subtle" onClick={showEstablecimiento} icon={<VisibleIcon />} />
      </Cell>
    );
  };
