...
function List() {
  ...
  const ActionCell = ({ rowData, dataKey, ...props }) => {
    function editDocType() {
      match(`/admin/sar/tipo_documento/editar/${rowData[dataKey]}`);
    }
    function showDocType() {
      match(`/admin/sar/tipo_documento/show/${rowData[dataKey]}`);
    }
    return (
      <Cell {...props} className="link-group">
        <IconButton appearance="subtle" onClick={editDocType} icon={<Edit2 />} />
        <Divider vertical />
        <IconButton appearance="subtle" onClick={showDocType} icon={<VisibleIcon />} />
      </Cell>
    );
  };
