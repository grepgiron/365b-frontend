...
const FormClient = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://beauty365api.herokuapp.com/api/v1/productos/'+id).
    then(res => {
      if(res.status === 200) {
        setFormValue({
          _id: res.data._id,
          ...
          unidad: `${res.data.unidad ? res.data.unidad._id : ''}`,
          costo: res.data.costo,
          precio: res.data.precio,
          categoria: `${res.data.categoria ? res.data.categoria._id : ''}`
        });
  ...
  const handleSubmit = async() => {
    ...
    if (!formRef.current.check()) {
      ...
      setLoading(false);
      try {
        const apiRes = await axios.put('https://beauty365api.herokuapp.com/api/v1/productos/'+formValue._id, 
    ...
    return (
      <>
        ...
        >
          ...
          <Form.Group>
            <ButtonToolbar>
              <Button appearance="primary" onClick={handleSubmit}>Actualizar</Button>
